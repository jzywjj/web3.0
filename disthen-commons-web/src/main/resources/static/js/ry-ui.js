    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $.table.responseHandler = $.common.isEmpty(options.responseHandler)?$.table.responseHandler:options.responseHandler;
                $('#'+options.id).bootstrapTable({
        	        ...
        	        toolbar: '#'+options.id+'toolbar',                                // 指定工作栏
                    sidePagination: "server",                           // 启用服务端分页
                    singleSelect:options.singleSelect,					//是否单选
                    onCheck:options.onCheck,							//复选框点击事件
                    onCheckAll:options.onCheckAll,						//复选框全选事件
                    onLoadSuccess:options.onLoadSuccess,				//数据加载完成事件
                    onUncheck:options.onUncheck,						//复选框取消选定事件
            ...
            search: function(formId) {
    		    ...
    		    params.queryParams = function(params) {
    		        ...
    		        search.page = params.offset / params.limit,
    		        search.sort = params.sort+","+params.order
    		        return search;
    		    }
    	...
    	modal: {
            ...
            // 弹出层指定宽度
            openNoCheck: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            // 保存信息
            save: function(url, data,id) {
            	$.modal.loading("正在处理中，请稍后...");
            	/*var str = data.split("&");
            	var param = "";
            	debugger;
            	for(let i in str){
            		if(str[i].indexOf("Time") != -1 || str[i].indexOf("Date") != -1){
            			var key = str[i].split("=");
            			if(key.length > 1){
            				if(key[1] != null && key[1] != ""){
            					debugger;
            					param += key[0]+"="+key[1]+" 00:00:00";
            				}
            			}
            		}else{
            			param += str[i];
