			...
			return jq.each(function(){
				...
				var draggableOptions = {
					...
					proxy: function(source) {
						...
						$.map(rows, function(row,i){
							...
							$('<td><span class="treegrid-dnd-icon treegrid-dnd-no" style="position:static">&nbsp;</span></td>').insertBefore(p.find('tr:eq('+i+') td:first'));
						});
				...
				var droppableOptions = {
					...
					onDragEnter: function(e, source){
						...
						if (dropTargetTree.treegrid("options").onDragEnter.call(target, getRow(this), getDraggingRow(source)) == false){
					...
					onDragOver: function(e, source) {
						...
						if (dropTargetTree.treegrid("options").onDragOver.call(target, this, getDraggingRow(source)) == false){
							setProxyFlag(source, false);
					...
					onDragLeave: function(e, source) {
						...
						$(this).css('border','');
						setProxyFlag(source, false);
						dropTargetTree.treegrid("options").onDragLeave.call(target, getRow(this), getDraggingRow(source));
					},
					onDrop: function(e, source) {
						...
						if (dropTargetTree.treegrid("options").onBeforeDrop.call(target, this, sRow, point) == false){
							return;
						}
						dropTargetTree.treegrid("options").onDrop.call(target, this, sRow, point);
					}
				}
				...
				function setProxyFlag(source, allowed){
					var icon = $(source).draggable('proxy').find('span.treegrid-dnd-icon');
					icon.removeClass('treegrid-dnd-yes treegrid-dnd-no').addClass(allowed ? 'treegrid-dnd-yes' : 'treegrid-dnd-no');
				}
				function getRow(tr){
				...
				function setDroppable(target){
					//可放下的目标
					var c = target.find("div.treegrid-node");
					if (c.length > 0)
					{
