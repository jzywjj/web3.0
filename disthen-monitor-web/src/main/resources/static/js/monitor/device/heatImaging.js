...
$(document).ready(function() {
	...
	//定时刷新页面数据
	interval = setInterval(updateHI, 5000);
});

function initailHICharts(){
...
function updateHI(){
	var selectedDevice=window.top.scopeToShare.selectedHI;
	cardId=window.top.scopeToShare.cardId;
	updateHINode(selectedDevice);
}

function updateHINode(node) {
	...
	cables && $.each(cables, function(index, ts) {
		...
		if (type == "HITemperature") {
			TMPoption.series[0].data[0] = ts.value;
			temperature.setOption(TMPoption);
		}
		if(data_time != ts.data_time){
			 var url="/mon_his/selectHIData?cardId="+cardId;
